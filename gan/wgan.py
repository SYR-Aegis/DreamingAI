...
from utils.dataset import celeba, celeba_generator
...
class GAN():
    ...
    def __init__(self, input_dim=(128, 128, 3), batch_size=32):
        ...
        self.weight_save_dir = "../data/weight/wgan/"
        self.img_save_dir = "../data/generated_images/wgan/"
    ...
    def conv_reg(self, img, filter, kernel, strides, conv_trans=False, batch_norm=True, active="leakyrelu"):
        ...
        if conv_trans == True:
            x = Conv2DTranspose(
                filters=filter, kernel_size=kernel, strides=strides, padding='same')(x)

        elif conv_trans == "upsample":
            x = UpSampling2D()(x)
            x = Conv2D(filters=filter, kernel_size=kernel, strides=1, padding='same')(x)
        else:
    ...
    def generator_model(self):
        ...
        x = self.conv_reg(x, 3, 5, 2, True, False, "tanh")
    ...
    def discriminator_model(self):
        ...
        x = self.conv_reg(x, 64, 5, 2, False, False)
        x = self.conv_reg(x, 128, 5, 2, False, False)
        x = self.conv_reg(x, 128, 5, 1, False, False)
    ...
    def save_image(self, epoch):
        ...
        save_img(self.img_save_dir+"generated_image_"+str(epoch)+".png", fake_img[0])
...
def celeba_data(path):

    DATA_PATH = path
    BATCH_SIZE = 32
    data_gen = ImageDataGenerator(rescale=1./255)
    data_flow = data_gen.flow_from_directory(DATA_PATH, target_size=[128, 128], batch_size=BATCH_SIZE, shuffle=True,
